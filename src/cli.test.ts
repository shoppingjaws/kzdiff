...
describe("kzdiff CLI", () => {
  ...
  describe("Basic functionality", () => {
    ...
    test("should compare with specific commit hash", async () => {
      const { stdout, exitCode } = await runCLI([EXAMPLE_PATH, "-r", TEST_COMMIT]);
    ...
    test("should pass kustomize options after --", async () => {
      const { stdout, exitCode } = await runCLI([EXAMPLE_PATH, "--", "--enable-helm"]);
  ...
  describe("Debug output", () => {
    test("should show debug output when DEBUG env is set", async () => {
      ...
      const { stdout, stderr } = await runCLI([EXAMPLE_PATH, "-r", TEST_COMMIT]);
