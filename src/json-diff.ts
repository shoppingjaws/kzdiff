...
function getContext(
  ...
  path: string, 
  contextSize = 2
): { before: Array<{ key: string; value: any }>, after: Array<{ key: string; value: any }> } {
...
function formatYamlPath(path: string, basePath: string, indent: string = "  "): string {
  if (!basePath) {
    return path;
  }
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (part.includes('[')) {
      ...
      if (i === parts.length - 1) {
        ...
        result.push(currentIndent + name + indices.map(idx => `[${idx}`).join(''));
      } else {
