...
export async function filterYaml(filePath: string, filters: string[]): Promise<void> {
	if (filters.length === 0) {
		return
	}
	...
	try {
		...
		debug(`Filtered ${documents.length} document(s) to ${filteredDocuments.length}`)

		// Write filtered YAML back to file
		let outputContent = ""
		if (filteredDocuments.length > 0) {
			outputContent = filteredDocuments.map((doc) => yaml.dump(doc, { noRefs: true })).join("---\n")
		}
